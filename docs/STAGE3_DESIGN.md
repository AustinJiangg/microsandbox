...
- [ ] **3b**：`backend="microvm"` 端到端跑通——从一个真 Firecracker microVM 里 `run_code`
      拿回结果；切 kernel 后端后变量跨 `run_code` 留存；`close` 后无残留进程/工作目录。
- [ ] **3c**：测量并记录冷启动时间；资源限制经 `machine-config` 生效；guest 不配网卡仍
      可管理（隔离反超已落实并在文档说明）。
