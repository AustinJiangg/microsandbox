...
// It also returns the writable *block.Overlay it bound and the build id it served the rootfs from
// (nil / "" in the legacy/non-NBD path) -- retained per sandbox since Stage 26R, so a later Pause can
// export the overlay's dirtied blocks and diff against that build. The overlay is the same object the
// returned Close tears down, so a caller must ExportToDiff before the VM's Destroy runs Close.
func (s *server) buildRootfsBacking(tmpl template.Template) (fc.RootfsBacking, *block.Overlay, string, error) {
