...
# Size the image from the exported content plus the writable margin. (A COW layered child is NOT built
# here -- it copies its base's rootfs and edits it in place via scripts/build-rootfs-layered.sh (Stage 19),
# inheriting the base's exact size and block layout; this script only ever builds a from-scratch image.)
